...
This directory contains CI workflows for SuperCodex Framework.

## Workflows

### 1. **test.yml** - Comprehensive Test Suite
**Triggers**: Push/PR to `main` or `master`, manual dispatch
**Jobs**:
...
- **doctor-check**: Run `supercodex doctor` health check
...
# Run with coverage
uv run pytest --cov=supercodex --cov-report=term
...
# Verify plugin loads
uv run pytest --trace-config | grep supercodex

# Run doctor check
uv run supercodex doctor --verbose
```
...
```bash
uv run pytest --cov=supercodex --cov-report=html
...
- [Testing Guide](../../docs/developer-guide/testing-debugging.md)
